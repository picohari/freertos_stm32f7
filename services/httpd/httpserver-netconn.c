...
static void http_server_netconn_thread( void const * args );
...
{ 
  ...
  LCD_UsrLog ((char *)"  State: HTTPD NETCON ok ...\n");
  
  if (conn!= NULL)
  {
    /* Bind to port 80 (HTTP) with default IP address */
    err = netconn_bind(conn, NULL, 80);
    LCD_UsrLog ((char *)"  State: HTTPD BIND ok ...\n");
    
    if (err == ERR_OK)
    {
      /* Put the connection into LISTEN state */
      netconn_listen(conn);
      LCD_UsrLog ((char *)"  State: HTTPD LISTEN ok ...\n");
...
void http_server_netconn_init()
{
  ...
  osThreadDef(httpd, http_server_netconn_thread, osPriorityNormal, 0, 512);
  osThreadCreate( osThread( httpd ), NULL );
}
...
void DynWebPage(struct netconn *conn)
{
  ...
  strcat((char *)PAGE_BODY, "---------------------------------------------<br>");
  strcat((char *)PAGE_BODY, "B : Blocked, R : Ready, D : Deleted, S : Suspended<br>");
