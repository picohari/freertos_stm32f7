...
#endif
{
  ...
  //TC3_COUNTER_CURRENT = 65536 - CLOCK_SECONDS;
  //TC3_COUNTER_COMPARE = 65536 - CLOCK_SECONDS + CLOCK_TICKS;
#endif
...
clock_set_time(timestamp_t new_sync_timestamp)
{
  ...
  if (sync_timestamp)
  {
    ...
    {
      uint32_t new_value = TC3_COUNTER_COMPARE;
      new_value *= NTP_RESYNC_PERIOD;
      new_value /= NTP_RESYNC_PERIOD + delta;

      NTPADJDEBUG("new OCR3A value %d\n", new_value);
      TC3_COUNTER_COMPARE = new_value;
    }
