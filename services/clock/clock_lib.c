...
#define UTCTIME   (uint32_t)(&clock_tz.utctime)
#define DSTTIME   (uint32_t)(&clock_tz.dsttime)
#define DSTBEGIN  &clock_tz.dstbegin
#define DSTEND    &clock_tz.dstend


const char clock_weekdays[] = {
  'S', 'u', 'n', 0,
  'M', 'o', 'n', 0,
  'T', 'u', 'e', 0,
  'W', 'e', 'd', 0,
...
const uint8_t clock_monthdays[] =
  { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };


static const uint16_t clock_monthydays[2][13] = {
  /* normal years  */
...
static const clock_timezone_t clock_tz = {
...
{
  uint16_t yday =
    (&clock_monthydays[IS_LEAP_YEAR(year)][month - 1]);
  return yday + day - 1;
}
                     ...
                     const uint8_t year)
{
  ...
  clock_dst_t dst;
  memcpy(&dst, dst_flash, sizeof(clock_dst_t));
